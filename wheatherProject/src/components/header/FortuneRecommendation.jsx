...
export default function FortuneRecommendation({ onClose }) {
    ...
    const validateInputs = useCallback(() => {
        if (!userInputs.mbti || !userInputs.birthYear || !userInputs.birthMonth ||
            !userInputs.birthDay || !userInputs.gender) {
    ...
    const calculateFortune = useCallback(() => {
        ...
        try {
            ...
            const timeOfDay =
                currentHour >= 5 && currentHour < 12 ? "아침" :
        ...
        [행운의 시간]
        ${timeOfDay}에는 ${timeOfDay === "아침" ? "새로운 시작을 계획하는 것이 좋습니다." :
                    timeOfDay === "오후" ? "적극적인 활동이 행운을 가져올 것입니다." :
                        timeOfDay === "저녁" ? "사람들과의 교류가 좋은 기회를 만들어낼 것입니다." :
    ...
    }, [userInputs, isSuccess, weatherData, mbtiTraits, monthlyTraits, genderTraits, validateInputs]);
    ...
    // 운세 모달 닫기
    const handleCloseFortuneModal = () => {
        setShowFortuneModal(false);
        onClose(); // 모달을 닫고 부모 컴포넌트에 알림
    };
    ...
    return (
        <>
            ...
            {showFortuneModal && fortune && (
                <FortuneModal
                    fortune={fortune}
                    onClose={handleCloseFortuneModal}
                />
            )}
        </>
    );
